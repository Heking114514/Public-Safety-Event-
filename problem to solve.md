...
- 当前进展：路线 CSV 解析、动态 `nav_msgs/Path` 转换、默认值校验和重复路线判定已抽到 `visual_navigation::RouteManager`；odom、IMU、fusion-status、tracking 和执行器健康的接收/新鲜度状态已抽到 `visual_navigation::NavigationInputCache`，并有独立单元测试。
