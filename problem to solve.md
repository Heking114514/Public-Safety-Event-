...
- 当前状态：活动 `src/arena_path_planner/config/arena_map.yaml` 已换成正式 `3.2 x 4.4 m` 赛场，并按实车配置 `0.1434 x 0.1437 m` 外廓、`0.015 m` 安全边距和 `0.020 m` 跟踪边距。所有平移路径段使用完整矩形 footprint 复检；窄路口的差速原地转向按比赛策略显式允许，避免保守外接矩形把可跑路线全部拒绝。`0.217 x 0.210 m` 只保留在不兼容地图测试中，原 6 x 6 m 地图保存在 `arena_map_6x6_legacy.yaml`，不再默认加载。
...
- 当前状态：停车确认优先使用刹车后新鲜 MCU 双轮速度，遥测缺失时回退融合位姿速度。单次确认超时会继续发布零速度并进入 `RECOVERING_WAYPOINT_BRAKE` 自动重试，不关闭任务；未确认停稳仍不会开始转向。
