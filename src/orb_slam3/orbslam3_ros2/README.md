...
- 当前工作空间按以下环境维护和验证：
  - Ubuntu 22.04
  - ROS2 Humble
  - 系统 OpenCV（Ubuntu 22.04 的发行版本即可）

- ORB-SLAM3、Pangolin 和 Sophus 已作为 `src/orb_slam3/` 下的源码依赖管理。
- 第一次在新环境构建时，请从工作空间根目录运行 `./x86_build.sh`；ARM64 板运行 `./arm_build.sh`。
...
## 构建方法
1. 进入本仓库工作空间根目录。新环境使用构建脚本：
```
$ cd /path/to/your/workspace
$ ./x86_build.sh
```

ARM64 板将 `./x86_build.sh` 替换为 `./arm_build.sh`。脚本会构建本地 Pangolin、Sophus、ORB-SLAM3 和 ROS 2 封装，并执行测试。

完成一次原生库构建后，仅修改 ROS 封装时可以直接增量构建：
```
$ cd /path/to/your/workspace
...
## 故障排查
1. 如果找不到 `sophus/se3.hpp`，请确认已先运行对应架构的工作空间构建脚本；脚本会把 Sophus 安装到被 `.gitignore` 忽略的本地 `src/orb_slam3/deps/` 前缀。

2. 如果 CMake 缓存来自其他工作空间，请删除工作空间的 `build/ install/ log/` 后重新运行构建脚本。不要提交这些目录。
...
词汇表和配置文件会安装到工作空间前缀（例如 `install/orbslam3/share/orbslam3/vocabulary/ORBvoc.txt` 和 `install/orbslam3/share/orbslam3/config/monocular/TUM1.yaml`）；D455 Launch 会自动使用这些文件。
  - `MONO` 模式  
```
...
ARM64 板将 `./x86_build.sh` 替换为 `./arm_build.sh`。该 Launch 文件会选择序列号为 `038122250473` 的相机以及 `848x480x30` 的红外数据流；上例显式启用 ORB-SLAM3 内部 IMU 融合执行双目-惯性尺度里程计。启动后的前两秒双目图像会被丢弃，以等待 D455 自动曝光稳定。红外发射器会被禁用，避免其投射的点阵图案干扰 ORB 特征提取；所需的话题重映射会自动应用。
...
启动时，以适中的幅度平移相机并绕多个轴转动，直到 `/diagnostics` 报告 `Tracking`。双目惯性模式下保持相机完全静止无法为惯性初始化提供足够的加速度变化；纯双目模式也需要图像视差和纹理。

如需禁用 IMU，请使用纯双目模式，并根据需要启用启动重置：

```bash
ros2 launch orbslam3 realsense_d455_stereo_inertial.launch.py \
  use_slam_imu:=false enable_imu:=false initial_reset:=true
```
...
enable_imu:=true          # 发布 D455 IMU 话题，供外部滤波或融合使用
...
2. 启动各终端：  
（例如，`ROS1_INSTALL_PATH`=`/opt/ros/noetic`，`ROS2_INSTALL_PATH`=`/opt/ros/humble`。）
```
# Shell A：
