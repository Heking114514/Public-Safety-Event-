...
public:
  ...
  void configure(
    double stop_speed, double stop_dwell, double minimum_stop_time, double timeout)
  {
  ...
  bool check(
    TimePoint now, bool speed_valid, double speed)
  {
    if (!stop_timer_initialized_) {
      stop_started_ = now;
      stop_timer_initialized_ = true;
    }
    const double total_stop_seconds = std::chrono::duration<double>(
      now - stop_started_).count();
    const double absolute_speed = speed_valid ? speed : std::numeric_limits<double>::quiet_NaN();
    ...
    const double settled_seconds = speed_settle_timer_initialized_ ?
      std::chrono::duration<double>(now - speed_settle_started_).count() : 0.0;
    const bool complete = WaypointStopSatisfied(
      absolute_speed, settled_seconds, total_stop_seconds, stop_speed_, stop_dwell_,
      minimum_stop_time_, timeout_);
